...
PRODUCT_PROPERTY_OVERRIDES += \
    ...
    ro.config.alarm_alert=Smoth.ogg
...
# Backup Tool
PRODUCT_COPY_FILES += \
    vendor/$(Base_Vendor_Name)/prebuilt/common/bin/backuptool.sh:install/bin/backuptool.sh \
    vendor/$(Base_Vendor_Name)/prebuilt/common/bin/backuptool.functions:install/bin/backuptool.functions \
    vendor/$(Base_Vendor_Name)/prebuilt/common/bin/50-cm.sh:system/addon.d/50-cm.sh \
    vendor/$(Base_Vendor_Name)/prebuilt/common/bin/blacklist:system/addon.d/blacklist

# Backup Services whitelist
PRODUCT_COPY_FILES += \
    vendor/$(Base_Vendor_Name)/config/permissions/backup.xml:system/etc/sysconfig/backup.xml

# Signature compatibility validation
PRODUCT_COPY_FILES += \
    vendor/$(Base_Vendor_Name)/prebuilt/common/bin/otasigcheck.sh:install/bin/otasigcheck.sh

# init.d support
PRODUCT_COPY_FILES += \
    vendor/$(Base_Vendor_Name)/prebuilt/common/etc/init.d/00banner:system/etc/init.d/00banner \
    vendor/$(Base_Vendor_Name)/prebuilt/common/bin/sysinit:system/bin/sysinit

ifneq ($(TARGET_BUILD_VARIANT),user)
# userinit support
PRODUCT_COPY_FILES += \
    vendor/$(Base_Vendor_Name)/prebuilt/common/etc/init.d/90userinit:system/etc/init.d/90userinit
endif

# CM-specific init file
PRODUCT_COPY_FILES += \
    vendor/$(Base_Vendor_Name)/prebuilt/common/etc/init.local.rc:root/init.cm.rc

# Copy over added mimetype supported in libcore.net.MimeUtils
PRODUCT_COPY_FILES += \
    vendor/$(Base_Vendor_Name)/prebuilt/common/lib/content-types.properties:system/lib/content-types.properties
...
# This is CM!
PRODUCT_COPY_FILES += \
    vendor/$(Base_Vendor_Name)/config/permissions/com.$(Base_Vendor_Name_full).android.xml:system/etc/permissions/com.$(Base_Vendor_Name_full).android.xml

# Include Lineage Light audio files
include vendor/light/config/lineage_light_audio.mk
...
# TWRP
ifeq ($(WITH_TWRP),true)
include vendor/$(Base_Vendor_Name)/config/twrp.mk
endif
...
DEVICE_PACKAGE_OVERLAYS += vendor/$(Base_Vendor_Name)/overlay/common
...
PRODUCT_EXTRA_RECOVERY_KEYS += \
    vendor/$(Base_Vendor_Name)/build/target/product/security/lineage

-include vendor/$(Base_Vendor_Name)-priv/keys/keys.mk
...
-include vendor/$(Base_Vendor_Name)/config/partner_gms.mk
